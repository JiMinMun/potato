...
# Read in the data
with open('./sample_for_human_eval_dartmouth.json', 'r') as f:
    data = json.load(f)

options = ['A', 'B', 'C', 'D', 'E', 'F', 'G']
sample_variants = ["human_output", "output_base", "output_dpo_data", "output_dpo_policy", "output_ppo_data", "output_ppo_reward", "output_ppo_policy"]
...
options_str = """A. {A}<br>B. {B}<br>C. {C}<br>D. {D}<br>E. {E}<br> F. {F}<br>G. {G}<br>"""
data_arr = []
...
for i, row in enumerate(data):
    ...
    sample.update({f"option_{options[i]}": row[k] for i, k in enumerate(shuffled_variants)})
    ...
    display_text_str = [context_str.format(patient_info=row['context'].replace('Patient: ', '').replace('\nDoctor:', '').replace('\n', '<br>')), q_options_str, cur_options_str]
    sample['text'] = display_text_str
    # sample['id'] = row['id']
    data_arr.append(sample)

jsonlines.open('./sample_for_human_eval_dartmouth_formatted.jsonl', 'w').write_all(data_arr)